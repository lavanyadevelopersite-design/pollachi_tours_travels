...
const connectDatabase = async () => {
  try {
    ...
    const ensureNameMasterTable = async (tableName, nameColumn) => {
      const [rows] = await sequelize.query(`SHOW TABLES LIKE '${tableName}'`);
      if (rows && rows.length > 0) return;
      await sequelize.query(`
        CREATE TABLE IF NOT EXISTS ${tableName} (
          id CHAR(36) NOT NULL PRIMARY KEY,
          ${nameColumn} VARCHAR(150) NOT NULL,
          ...
          UNIQUE KEY uq_${tableName}_${nameColumn} (${nameColumn})
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
      `);
      logger.info(`Created ${tableName} table`);
    };

    const alignNameMasterCollation = async (tableName) => {
      const [rows] = await sequelize.query(`SHOW TABLES LIKE '${tableName}'`);
      if (!rows || rows.length === 0) return;
      await sequelize.query(
    ...
    try {
      await ensureNameMasterTable('agent_master', 'agent_name');
      await ensureNameMasterTable('corporate_master', 'corporate_name');
      await alignNameMasterCollation('agent_master');
      await alignNameMasterCollation('corporate_master');

      const { randomUUID } = require('crypto');
      const masterModules = ['agents', 'corporates'];
      const masterActions = ['create', 'edit', 'delete', 'approve', 'export', 'print', 'view'];
      for (const module of masterModules) {
        for (const action of masterActions) {
          ...
          await sequelize.query(
            `INSERT INTO permissions (id, name, code, module, action, description, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
            {
              replacements: [
                randomUUID(),
      ...
      logger.warn('Could not ensure agent/corporate master tables:', masterErr.message);
    }
      ...
      if (!rows || rows.length === 0) {
        await sequelize.query(`
          CREATE TABLE IF NOT EXISTS enquiry_vehicle_assignments (
            ...
            notes TEXT NULL,
            share_code VARCHAR(12) NULL,
            vendor_driver_name VARCHAR(150) NULL,
            created_by CHAR(36) NULL,
            updated_by CHAR(36) NULL,
            created_at DATETIME NOT NULL,
            updated_at DATETIME NOT NULL,
            ...
            UNIQUE KEY uq_eva_share_code (share_code)
        ...
        try {
          await sequelize.query(
            'CREATE UNIQUE INDEX uq_eva_share_code ON enquiry_vehicle_assignments (share_code)'
          );
        } catch {
          // Index already exists or share_code not unique yet
        }
      }
    } catch (tableErr) {
    ...
    // Track which lead statuses were actually set so skipped stages can be coloured.
    try {
      const [enqTable] = await sequelize.query("SHOW TABLES LIKE 'enquiries'");
      if (enqTable && enqTable.length > 0) {
        const [visitedCol] = await sequelize.query(
          "SHOW COLUMNS FROM enquiries LIKE 'visited_lead_status_ids'"
        );
        if (!visitedCol || visitedCol.length === 0) {
          await sequelize.query(
            'ALTER TABLE enquiries ADD COLUMN visited_lead_status_ids JSON NULL AFTER lead_status_id'
          );
          logger.info('Added enquiries.visited_lead_status_ids');
        }
        const [countryNameCol] = await sequelize.query(
          "SHOW COLUMNS FROM enquiries LIKE 'country_name'"
        );
        if (!countryNameCol || countryNameCol.length === 0) {
          await sequelize.query(
            'ALTER TABLE enquiries ADD COLUMN country_name VARCHAR(150) NULL AFTER country_id'
          );
          logger.info('Added enquiries.country_name');
        }
        const partyColumns = [
          ['agent_id', 'CHAR(36) NULL AFTER enquiry_type'],
          ['corporate_id', 'CHAR(36) NULL AFTER agent_id'],
        ];
        for (const [column, definition] of partyColumns) {
          const [colRows] = await sequelize.query(
            `SHOW COLUMNS FROM enquiries LIKE '${column}'`
          );
          if (!colRows || colRows.length === 0) {
            await sequelize.query(
              `ALTER TABLE enquiries ADD COLUMN ${column} ${definition}`
            );
            logger.info(`Added enquiries.${column}`);
          }
        }
      }
    } catch (visitedColErr) {
      logger.warn('Could not ensure enquiries.visited_lead_status_ids:', visitedColErr.message);
    }
    ...
    try {
      const [payCols] = await sequelize.query(
        "SHOW COLUMNS FROM enquiry_payments LIKE 'additional_charges'"
      );
      if (!payCols || payCols.length === 0) {
        await sequelize.query(
          'ALTER TABLE enquiry_payments ADD COLUMN additional_charges DECIMAL(12,2) NULL DEFAULT 0.00 AFTER advance_amount'
        );
        logger.info('Added enquiry_payments.additional_charges');
      }
    } catch (payColErr) {
      logger.warn('Could not ensure enquiry_payments.additional_charges:', payColErr.message);
    }
    ...
    try {
      const noteColumns = [
        ...
        ['lead_status_color', 'VARCHAR(20) NULL'],
      ];
      for (const [column, definition] of noteColumns) {
        const [colRows] = await sequelize.query(
          `SHOW COLUMNS FROM enquiry_notes LIKE '${column}'`
        );
        if (!colRows || colRows.length === 0) {
          await sequelize.query(
            `ALTER TABLE enquiry_notes ADD COLUMN ${column} ${definition}`
          );
          logger.info(`Added enquiry_notes.${column}`);
        }
      }
    } catch (noteColErr) {
      logger.warn('Could not ensure enquiry_notes stage columns:', noteColErr.message);
    }
  } catch (error) {
