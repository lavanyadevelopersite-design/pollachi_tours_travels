...
const getTripRoute = asyncHandler(async (req, res) => {
  const data = await driverPortalService.getMyTripRoute(req.user, req.params.id);
  res.json(ApiResponse.success('Trip route retrieved', data));
});
...
const getTripByShareCode = asyncHandler(async (req, res) => {
  const data = await driverPortalService.getMyTripByShareCode(req.user, req.params.code);
  res.json(ApiResponse.success('Trip retrieved', data));
});
