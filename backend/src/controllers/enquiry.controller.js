...
module.exports = {
  ...
  searchPlaces: asyncHandler(async (req, res) => {
    const result = await enquiryService.searchPlaces(req.query.q, req.query.type);
  ...
  resolveDriverShare: asyncHandler(async (req, res) => {
    const result = await enquiryService.resolveDriverShare(req.params.code);
    res.json(ApiResponse.success('Driver share link resolved', result));
  }),
  ...
  listCustomerWhatsAppMessages: asyncHandler(async (req, res) => {
    const rows = await enquiryService.listCustomerWhatsAppMessages(req.params.id);
    res.json(ApiResponse.success('WhatsApp messages retrieved', rows));
  }),

  sendCustomerWhatsApp: asyncHandler(async (req, res) => {
    const result = await enquiryService.sendCustomerWhatsApp(
      req.params.id,
      req.body.message,
      req.user?.id || null
    );
    if (result?.success) {
      res.json(ApiResponse.success('Message sent on WhatsApp', result));
      return;
    }
    ...
    const reasonMessages = {
      ...
      wasender_disabled:
        'WhatsApp is not configured. Open Integrations → WhatsApp, save the API key, and scan the QR code.',
      session_not_connected:
        'WhatsApp session is not connected. Open Integrations → WhatsApp and scan the QR code.',
      wasender_unreachable:
        'Could not reach the WhatsApp API. Check the server internet connection and try again.',
  ...
  listCustomers: asyncHandler(async (req, res) => {
    const rows = await enquiryService.listCustomers(req.query);
    res.json(ApiResponse.success('Customers retrieved', rows));
  }),

  findCustomerByPhone: asyncHandler(async (req, res) => {
    const customer = await enquiryService.findCustomerByPhone(req.query.phone);
    res.json(ApiResponse.success('Customer retrieved', customer));
  }),

  getPipelineStats: asyncHandler(async (req, res) => {
    const result = await enquiryService.getPipelineStats(req.query);
    res.json(ApiResponse.success('Enquiry pipeline stats retrieved', result));
  }),
};
