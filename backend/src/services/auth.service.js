...
const logout = async (accessToken, refreshToken, userId) => {
  if (refreshToken) {
    await tokenService.revokeRefreshToken(refreshToken, { endSession: true, reason: 'logout' });
  } else if (userId) {
    await loginHistoryService.endOpenSessionsForUser(userId, 'logout');
  }
  if (accessToken) {
    await tokenService.blacklistToken(accessToken, userId, dayjs().add(1, 'day').toDate(), 'logout');
  }
  return true;
};
