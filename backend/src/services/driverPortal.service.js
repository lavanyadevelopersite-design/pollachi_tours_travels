...
const { mergeVisitedLeadStatusIds, sortLeadStatuses } = require('../utils/leadStatusPipeline');
...
const recordDriverStatusLog = async ({
  ...
  const assignment = await EnquiryVehicleAssignment.findByPk(assignmentId, {
    attributes: ['id', 'trip_status_history'],
  });
  if (!assignment) return;

  const history = parseStatusHistory(assignment.trip_status_history);
  history.push({
    status,
    label,
    at: recordedAt.toISOString(),
  });
  await assignment.update({ trip_status_history: history });
};
...
const serializeTrip = (row) => {
  ...
  return {
    ...
    trip_status_label: data.trip_status
      ...
      : 'Assigned',
    enquiry_lead_status: enquiry.leadStatus?.lead_status || null,
    ...
    enquiry: enquiry.id
      ? {
          ...
          lead_status: enquiry.leadStatus?.lead_status || null,
          travel_from_destination: enquiry.travel_from_destination || null,
          travel_to_destination: enquiry.travel_to_destination || null,
          travel_from_lat: enquiry.travel_from_lat != null ? Number(enquiry.travel_from_lat) : null,
          travel_from_lng: enquiry.travel_from_lng != null ? Number(enquiry.travel_from_lng) : null,
          travel_to_lat: enquiry.travel_to_lat != null ? Number(enquiry.travel_to_lat) : null,
          travel_to_lng: enquiry.travel_to_lng != null ? Number(enquiry.travel_to_lng) : null,
        }
      : null,
...
const getMyTripRoute = async (user, tripId) => {
  const trip = await getMyTrip(user, tripId);
  const status = String(trip.trip_status || '');
  if (!['trip_ongoing', 'trip_closed'].includes(status)) {
    throw new AppError('Route map is available after Trip ongoing', 400);
  }

  const pickup = trip.pickup_location;
  const drop = trip.drop_location;
  if (!pickup || !drop) {
    throw new AppError('Pickup and drop locations are required to show the route', 400);
  }

  const enquiry = trip.enquiry || {};
  const route = await distanceService.getDrivingRoute({
    fromLat: enquiry.travel_from_lat,
    fromLng: enquiry.travel_from_lng,
    toLat: enquiry.travel_to_lat,
    toLng: enquiry.travel_to_lng,
    fromPlace: pickup,
    toPlace: drop,
  });
  ...
  return {
    trip_id: trip.id,
    enquiry_code: trip.enquiry_code,
    ...
    from: {
      lat: route.from.lat,
      lng: route.from.lng,
      label: route.from.label || pickup,
    },
    to: {
      lat: route.to.lat,
      lng: route.to.lng,
      label: route.to.label || drop,
    },
    geometry: route.geometry,
  };
};

const getMyTripByShareCode = async (user, shareCode) => {
  const driverId = await resolveDriverId(user);
  const code = String(shareCode || '').trim();
  if (!code) throw new AppError('Share code is required', 400);
  ...
  if (!row) {
    throw new AppError('No trip found for this share link', 404);
  }
...
const logDriverUpdateOnEnquiry = async (assignment, user, updates, previous = {}) => {
  ...
  await enquiryService.addNote(assignment.enquiry_id, lines.join('\n'), user.id);
};
...
const syncEnquiryLeadStatusFromDriverTrip = async (enquiryId, userId, targetName) => {
  if (!enquiryId || !targetName) return null;
  ...
  const targetStatus = sorted.find(
    (status) => normalizeLeadName(status.lead_status) === normalizeLeadName(targetName)
  );
  if (!targetStatus) return null;

  const currentIndex = sorted.findIndex((status) => status.id === enquiry.lead_status_id);
  const targetIndex = sorted.findIndex((status) => status.id === targetStatus.id);
  if (targetIndex < 0) return null;
  if (currentIndex >= targetIndex) return null;

  const currentName = enquiry.leadStatus?.lead_status || '';
  await enquiry.update({
    lead_status_id: targetStatus.id,
    visited_lead_status_ids: mergeVisitedLeadStatusIds({
      ...
      currentName,
      nextId: targetStatus.id,
    }),
    ...(userId ? { updated_by: userId } : {}),
  });

  const enquiryService = require('./enquiry.service');
  await enquiryService.addNote(
    enquiryId,
    `Lead status auto-updated to ${targetStatus.lead_status} after driver trip update (was ${currentName || '—'})`,
    userId || null
  );

  try {
    if (normalizeLeadName(targetStatus.lead_status) === 'trip completed') {
      await enquiryService.sendTripCompletedInvoiceWhatsApp(enquiryId);
    } else {
      await whatsappNotification.sendForLeadStatusChange(enquiry, targetStatus);
    }
  } catch (err) {
    logger.warn('WhatsApp notification failed after driver trip status sync', {
      enquiryId,
      targetStatus: targetStatus.lead_status,
      message: err.message,
    });
  }

  logger.info('Enquiry lead status synced from driver portal', {
    enquiryId,
    previousStatus: currentName,
    nextStatus: targetStatus.lead_status,
  });
...
const updateMyTrip = async (user, tripId, payload = {}, files = {}) => {
  ...
  if (row.trip_status === 'trip_closed' || row.status === 'completed') {
    throw new AppError('This trip is completed and cannot be updated', 400);
  }
  ...
  if (startingKmLocked) {
    ...
    if (payload.starting_km !== undefined) {
      if (!hasKmValue(payload.starting_km)) {
        throw new AppError('Starting KM cannot be cleared after it has been saved', 400);
      }
      if (Number(payload.starting_km) !== Number(row.starting_km)) {
  ...
  if (payload.trip_status) {
    ...
    if (statusDef.requiresStartingKm || payload.trip_status === 'trip_ongoing') {
      if (!hasKmValue(startingKm)) {
        throw new AppError('Enter starting KM before moving to Trip ongoing', 400);
      }
      if (!effectiveStartingPhoto) {
        throw new AppError('Starting KM screenshot is required before Trip ongoing', 400);
      }
    }
    if (statusDef.requiresClosingKm || payload.trip_status === 'trip_closed') {
      if (!hasKmValue(startingKm)) {
        throw new AppError('Starting KM must be set before closing the trip', 400);
      }
      if (!hasKmValue(closingKm)) {
        throw new AppError('Enter closing KM before moving to Trip closed', 400);
      }
      if (Number(closingKm) < Number(startingKm)) {
        throw new AppError('Closing KM cannot be less than starting KM', 400);
      }
    }
  ...
  if (nextTripStatus && nextTripStatus !== previous.trip_status) {
    ...
    if (nextTripStatus === 'trip_ongoing') {
      await syncEnquiryLeadStatusFromDriverTrip(row.enquiry_id, user.id, 'Trip Ongoing');
    }
    if (nextTripStatus === 'trip_closed') {
      await syncEnquiryLeadStatusFromDriverTrip(row.enquiry_id, user.id, 'Trip Completed');
    }
  }
