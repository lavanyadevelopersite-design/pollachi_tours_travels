...
} = require('../utils/leadStatusPipeline');
const { makeShareCode, ensureAssignmentShareCode } = require('../utils/shareCode');
...
const attachEnquiryParty = async (record) => {
  if (!record) return record;
  const [agent, corporate] = await Promise.all([
    record.agent_id
      ? Agent.findByPk(record.agent_id, { attributes: ['id', 'agent_name'] })
      : null,
    record.corporate_id
      ? Corporate.findByPk(record.corporate_id, { attributes: ['id', 'corporate_name'] })
      : null,
  ]);
  record.setDataValue('agent', agent);
  record.setDataValue('corporate', corporate);
  return record;
};

const getById = async (id) => attachEnquiryParty(await base.getById(id));
...
const applyEnquiryPartyFields = (payload = {}) => {
  const data = { ...payload };
  const type = data.enquiry_type;
  if (data.agent_id === '') data.agent_id = null;
  if (data.corporate_id === '') data.corporate_id = null;
...
const assignmentStatusPatch = async (record, assignedTo) => {
  if (!assignedTo) return {};
  const currentName = record.leadStatus?.lead_status || '';
  if (currentName && !isNewEnquiryStatus(currentName)) return {};

  const assignStatus = await LeadStatus.findOne({
    where: {
      is_active: true,
      lead_status: { [Op.like]: 'Assign Enquiry' },
    },
  });
  if (!assignStatus || record.lead_status_id === assignStatus.id) return {};

  const allStatuses = await LeadStatus.findAll({ where: { is_active: true } });
  return {
    lead_status_id: assignStatus.id,
    visited_lead_status_ids: mergeVisitedLeadStatusIds({
      previousVisited: record.visited_lead_status_ids,
      allStatuses,
      currentId: record.lead_status_id,
      currentName,
      nextId: assignStatus.id,
    }),
  };
};
...
const resolveCountryFromName = async (payload = {}) => {
  const name = String(payload.country_name || '').trim();
  ...
  else if (!payload.country_id) payload.country_id = null;
  return payload;
};
...
const create = async (payload, userId = null) => {
  ...
  const safePayload = await resolveCountryFromName(applyEnquiryPartyFields({ ...payload }));
  delete safePayload.lead_status_id;
  let data = await enrichCalculations({
    ...safePayload,
    lead_status_id: leadStatus.id,
    visited_lead_status_ids: [leadStatus.id],
  ...
  setImmediate(() => {
    whatsappNotification.sendTripSuccessWhatsApp(enquiry).catch((err) => {
      logger.warn('WhatsApp notification failed after enquiry create', {
...
const update = async (id, payload, userId = null) => {
  ...
  const isAssignOnly =
    ...
    payload.enquiry_type === undefined;

  if (!isAssignOnly && isEnquiryRecordLocked(record)) {
    throw new AppError('Enquiry cannot be edited after booking is confirmed', 403);
  }

  if (isAssignOnly) {
    const assignedTo = payload.assigned_to || null;
    const statusPatch = await assignmentStatusPatch(record, assignedTo);
    await record.update({
  ...
  let data = await enrichCalculations(
    applyEnquiryPartyFields(await resolveCountryFromName({ ...payload }))
  );
  ...
  if (data.assigned_to && data.assigned_to !== record.assigned_to) {
    Object.assign(data, await assignmentStatusPatch(record, data.assigned_to));
  }
...
const remove = async (id) => {
  const record = await base.getById(id);
  if (isEnquiryRecordLocked(record)) {
    throw new AppError('Enquiry cannot be deleted after booking is confirmed', 403);
  }
...
const updateStatus = async (id, { status, lead_status_id }, userId = null, roleCode = null) => {
  ...
  if (status !== undefined && status !== null && status !== '') {
    ...
    if (String(status).toLowerCase() === 'cancelled') {
      const leadName = record.leadStatus?.lead_status || '';
      if (/cancell?ed/i.test(String(record.status || '')) || /cancell?ed/i.test(leadName)) {
        throw new AppError('Enquiry is already cancelled', 400);
      }
      if (isCompletedLeadStatus(leadName) || isTripCompletedStatus(leadName)) {
        throw new AppError('Enquiry cannot be cancelled after the trip is completed', 403);
      }
  ...
  if (lead_status_id !== undefined && lead_status_id !== null && lead_status_id !== '') {
    ...
    if (lead_status_id !== record.lead_status_id) {
      try {
        assertLeadStatusTransition({
          ...
          nextName: leadStatus.lead_status,
          roleCode,
        });
      } catch (err) {
    ...
    data.visited_lead_status_ids = mergeVisitedLeadStatusIds({
      previousVisited: record.visited_lead_status_ids,
      allStatuses,
      currentId: record.lead_status_id,
      currentName: record.leadStatus?.lead_status,
    ...
    if (releaseResources) {
      try {
        ...
        logger.warn('Failed to release resources after lead status update', {
          enquiryId: id,
          message: err.message,
        });
      }
        ...
        Promise.resolve()
          .then(async () => {
            if (isTripCompletedStatus(leadStatus.lead_status)) {
              return sendTripCompletedInvoiceWhatsApp(id);
            }
            if (isBookingConfirmedStatus(leadStatus.lead_status)) {
              return whatsappNotification.sendBookingConfirmedWhatsApp({ id }, leadStatus);
            }
            const fullEnquiry = await base.getById(id);
            return whatsappNotification.sendForLeadStatusChange(fullEnquiry, leadStatus);
          })
          .catch((err) => {
...
const searchPlaces = (query, placeType) =>
  distanceService.searchPlaces(query, { limit: 8, placeType });
...
const noteStageSnapshot = (enquiry) => ({
  lead_status_id: enquiry?.lead_status_id || enquiry?.leadStatus?.id || null,
  lead_status_name: enquiry?.leadStatus?.lead_status || 'New Enquiry',
  lead_status_color: enquiry?.leadStatus?.button_color || '#3B82F6',
});
...
const getHistory = async (enquiryId) => {
  ...
  notes.forEach((n) => {
    const stage = n.lead_status_name ? `[${n.lead_status_name}] ` : '';
    rows.push({
      id: n.id,
      date: n.created_at || n.createdAt,
      action: 'Note',
      details: `${stage}${n.note}`,
...
const listVehicleAssignments = async (enquiryId) => {
  ...
  for (const row of rows) {
    if (!row.share_code) {
      await ensureAssignmentShareCode(row);
    }
  }
...
const addVehicleAssignment = async (enquiryId, payload, userId = null) => {
  ...
  const isVendorVehicle = String(vehicle.ownership || '').toLowerCase() === 'vendor';
  const vendorDriverName = isVendorVehicle
    ? String(payload.vendor_driver_name || '').trim() || null
    : null;
  if (isVendorVehicle && !vendorDriverName) {
    throw new AppError('Driver name is required for vendor vehicles', 400);
  }

  let driverId = isVendorVehicle ? null : payload.driver_id || null;
  if (driverId) {
  ...
  const assignmentPayload = {
    ...
    driver_id: driverId,
    vendor_driver_name: vendorDriverName,
...
const resolveDriverShare = async (code) => {
  const shareCode = String(code || '').trim();
  if (!shareCode) throw new AppError('Share code is required', 400);

  const assignment = await EnquiryVehicleAssignment.findOne({
    where: { share_code: shareCode },
    include: [{ model: Enquiry, as: 'enquiry', attributes: ['id', 'enquiry_code'] }],
  });
  if (!assignment) throw new AppError('Share link is invalid or expired', 404);

  return {
    enquiry_code: assignment.enquiry?.enquiry_code || null,
...
const buildInvoiceShareContext = async (enquiryId, payload = {}) => {
  ...
  const payments = await Payment.findAll({
    where: { enquiry_id: enquiryId },
    attributes: ['advance_amount', 'quotation_amount', 'additional_charges'],
    order: [['created_at', 'DESC']],
  });
  const paidAmount = Math.round(
    payments.reduce(
      (sum, row) =>
        sum + (Number(row.advance_amount) || 0) + (Number(row.additional_charges) || 0),
      0
    )
  );
  ...
  return {
    ...
    trip_from: enquiryPlain.travel_from || quotationPlain?.travel_from || itineraryPlain?.from_date || null,
    trip_to: enquiryPlain.travel_to || quotationPlain?.travel_to || itineraryPlain?.to_date || null,
  };
};
...
const maybeMarkFullyPaid = async (enquiryId, userId = null) => {
  ...
  await enquiry.update({
    ...
    visited_lead_status_ids: mergeVisitedLeadStatusIds({
      previousVisited: enquiry.visited_lead_status_ids,
      allStatuses,
      currentId: enquiry.lead_status_id,
      currentName,
      nextId: fullyPaidStatus.id,
    }),
    ...(userId ? { updated_by: userId } : {}),
  });
...
const updateVehicleAssignment = async (assignmentId, payload, userId = null) => {
  ...
  const nextVehicleId = payload.vehicle_id || record.vehicle_id;
  const vehicle = await Vehicle.findByPk(nextVehicleId);
  if (!vehicle || vehicle.is_active === false) throw new AppError('Vehicle not found', 404);

  const isVendorVehicle = String(vehicle.ownership || '').toLowerCase() === 'vendor';
  let driverId = payload.driver_id !== undefined ? payload.driver_id : record.driver_id;
  let vendorDriverName =
    payload.vendor_driver_name !== undefined
      ? String(payload.vendor_driver_name || '').trim() || null
      : record.vendor_driver_name;
  ...
  await markOnTrip({ vehicleId: record.vehicle_id, driverId: record.driver_id });
  if (previousVehicleId && previousVehicleId !== record.vehicle_id) {
    await setAvailableIfIdle({ vehicleId: previousVehicleId, excludeEnquiryId: record.enquiry_id });
  }
  if (previousDriverId && previousDriverId !== record.driver_id) {
...
const getMonthlyTrips = async ({ vehicleId = null, driverId = null, year, month } = {}) => {
  ...
  const trips = [
    ...enquiryTrips.map((row) => ({
      ...
      vehicle: row.vehicle,
      driver: row.driver,
      vendor_driver_name: row.vendor_driver_name || null,
      enquiry: row.enquiry,
      booking: null,
...
const getPipelineStats = async (query = {}) => getEnquiryPipelineStats(query);

const CUSTOMER_LOOKUP_ATTRS = [
  'id',
  'customer_name',
  'phone',
...
const listCustomers = async (query = {}) => {
  ...
  if (search) {
    const phoneMatch =
      searchDigits.length >= 10
        ? { phone: { [Op.like]: `%${searchDigits.slice(-10)}%` } }
        : { phone: { [Op.like]: `%${search}%` } };
    where[Op.and].push({
      [Op.or]: [{ customer_name: { [Op.like]: `%${search}%` } }, phoneMatch],
    });
  }
  ...
  for (const row of rows) {
    ...
    if (!customer || seen.has(customer.phone)) continue;
...
const findCustomerByPhone = async (phone) => {
  ...
  const match = rows.find((row) => phoneDigits(row.phone).endsWith(last10));
  return match ? mapCustomerRow(match) : null;
};

const sendCustomerWhatsApp = async (enquiryId, message, userId = null) => {
  const enquiry = await base.getById(enquiryId);
  return whatsappNotification.sendCustomCustomerWhatsApp({ enquiry, message, userId });
};

const listCustomerWhatsAppMessages = async (enquiryId) => {
  const enquiry = await base.getById(enquiryId);
  const phone = normalizeWhatsAppPhone(enquiry.phone);
  const conversationWhere = [{ enquiry_id: enquiryId }];
  if (phone) conversationWhere.push({ phone_number: phone });

  try {
    const conversations = await WhatsAppConversation.findAll({
      where: { [Op.or]: conversationWhere },
      attributes: ['id'],
    });
    const conversationIds = conversations.map((row) => row.id);

    const messageWhere = [{ enquiry_id: enquiryId }];
    if (conversationIds.length) {
      messageWhere.push({ conversation_id: { [Op.in]: conversationIds } });
    }
    ...
    logger.warn('Could not list customer WhatsApp messages', {
      enquiryId,
      error: error.message,
    });
    return [];
