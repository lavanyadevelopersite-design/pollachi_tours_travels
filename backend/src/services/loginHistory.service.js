...
const ONLINE_ACTIVITY_MINUTES = 20;
...
const endOpenSessionsForUser = async (userId, reason = 'timeout', logoutAt = null) => {
  if (!userId) return 0;
  const [count] = await LoginHistory.update(
    { logout_at: logoutAt || new Date(), logout_reason: reason },
...
const listByUser = async (query = {}) => {
  ...
  sessions.forEach((session) => {
    ...
    if (!byUser.has(key)) {
      ...
      byUser.set(key, {
        ...
        last_update_at: session.logout_at || session.updatedAt || session.login_at,
        logout_at: session.logout_at || null,
        logout_reason: session.logout_reason || null,
        latest_login_at: session.login_at,
        type: session.device || parseDevice(session.user_agent),
        working_hours_ms: 0,
        sessions_count: 0,
        is_online: false,
      });
    ...
    if (dayjs(session.login_at).isBefore(dayjs(row.first_login_at))) {
      row.first_login_at = session.login_at;
    }
    if (dayjs(session.login_at).isAfter(dayjs(row.latest_login_at))) {
      row.latest_login_at = session.login_at;
      row.logout_at = session.logout_at || null;
      row.logout_reason = session.logout_reason || null;
    }
    const updateAt = session.logout_at || session.updatedAt || session.login_at;
    if (dayjs(updateAt).isAfter(dayjs(row.last_update_at))) {
      row.last_update_at = updateAt;
      row.type = session.device || parseDevice(session.user_agent) || row.type;
    }
  });
  ...
  const sortField =
    sortBy === 'username' || sortBy === 'employee_name' || sortBy === 'name' ? 'username' : sortBy;
  aggregated.sort((a, b) => {
    let av = a[sortField];
    let bv = b[sortField];
    ...
    if (sortField === 'first_login_at' || sortField === 'last_update_at' || sortField === 'logout_at') {
