...
const withPaidSummary = async (record) => {
  ...
  if (enquiryId) {
    const siblings = await Payment.findAll({
      where: { enquiry_id: enquiryId },
      attributes: ['id', 'advance_amount', 'additional_charges', 'created_at', 'payment_code'],
      order: [
        ['created_at', 'ASC'],
        ['payment_code', 'ASC'],
      ],
  ...
  const additionalCharges = Number(plain.additional_charges) || 0;
  plain.already_paid = alreadyPaid;
  plain.additional_charges = additionalCharges;
  plain.received_total = receivedAmount + additionalCharges;
...
const create = async (payload, userId = null, file = null) => {
  ...
  data.additional_charges = Math.max(Number(data.additional_charges) || 0, 0);
