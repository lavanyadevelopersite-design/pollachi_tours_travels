...
const generatePaymentReceiptPdfBuffer = ({ payment = {}, branding = {} }) =>
  new Promise((resolve, reject) => {
    try {
      ...
      const additionalCharges = Number(payment.additional_charges) || 0;
      const receivedTotal = received + additionalCharges;
      ...
      if (additionalCharges > 0) {
        row('Additional Charges', formatMoney(additionalCharges));
        row('Total Received', formatMoney(receivedTotal), '#047857');
      }
      ...
      doc
        ...
        .fillColor('#374151')
        .text(
          `Received with thanks from ${enquiry.customer_name || 'Customer'} a sum of ${formatMoney(receivedTotal)}.`,
          { align: 'left' }
        );
      doc.moveDown(0.35);
