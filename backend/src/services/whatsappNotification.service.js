...
const { buildDriverShortLink, ensureAssignmentShareCode } = require('../utils/shareCode');
...
const extractWasenderMsgId = (result = {}) =>
  result?.data?.msgId ||
  result?.data?.id ||
...
const recordCustomerWhatsApp = async ({
  enquiry,
  phone,
  message,
  result,
  ...
  if (!result?.success || !phone || !message) return;
  try {
    const inbox = require('./whatsappInbox.service');
    await inbox.recordOutboundCustomerMessage({
      phoneNumber: phone,
      message: String(message),
      enquiryId: enquiry?.id || null,
      contactName: enquiry?.customer_name || null,
      userId,
      wasenderMsgId: extractWasenderMsgId(result),
      status: 'sent',
      messageType,
    });
  } catch (err) {
    logger.warn('Could not record customer WhatsApp message', {
...
const buildDriverPortalLoginLink = (enquiry = {}, assignment = {}) => {
  if (assignment?.share_code) {
    return buildDriverShortLink(assignment.share_code);
  }
  const base = String(
...
const sendTemplateWhatsApp = async ({
  ...
  if (result.success) {
    ...
    await recordCustomerWhatsApp({ enquiry: data, phone, message, result });
...
const sendTripSuccessWhatsApp = async (enquiry = {}) => {
  const templateConfig =
    (await whatsappTemplateService.getByTemplateName('Trip_Sucess')) ||
    (await whatsappTemplateService.getByTemplateName('Trip_Success'));
  if (!templateConfig?.template_content) {
    return { skipped: true, reason: 'no_trip_success_template' };
  }
...
const buildVehicleDriverAssignedVariables = async (enquiry = {}, assignment = {}) => {
  ...
  const extraValues = {
    ...
    driver_name: driver.full_name || assignment?.vendor_driver_name || '—',
...
const sendDriverLoginCredentialsWhatsApp = async ({
  ...
  await ensureAssignmentShareCode(assignment);
...
const buildItineraryVariables = async ({ enquiry = {}, itinerary = {}, shareUrl = '' }) => {
  ...
  const destination =
    ...
    '—';
  const fromDate = enquiryData.travel_from || itinerary.from_date || '';
  const toDate = enquiryData.travel_to || itinerary.to_date || '';
  const branding = await masterService.getBranding();
...
const sendItineraryWhatsApp = async ({ enquiry = {}, itinerary = {}, shareUrl = '' }) => {
  ...
  if (result.success) {
    ...
    const sentMessage = result.fullMessage || fullMessage;
    await recordCustomerWhatsApp({
      enquiry,
      phone,
      message: sentMessage,
      result,
...
const buildPaymentReceiptVariables = async (payment = {}) => {
  ...
  const companyPhone = (await resolveCompanyPhone()) || '';
  const paidAmount =
    (Number(payment.advance_amount) || 0) + (Number(payment.additional_charges) || 0);
...
const sendCustomCustomerWhatsApp = async ({ enquiry, message, userId = null }) => {
  const data = await resolveEnquiryPayload(enquiry);
  const phone = resolveCustomerPhone(data);
  if (!phone) {
    return { skipped: true, reason: 'missing_customer_phone' };
  ...
  if (result.success) {
    logger.info('Custom WhatsApp sent to customer', {
      enquiryId: data.id,
      enquiryCode: data.enquiry_code,
      customerPhone: phone,
      to: result.to,
    ...
    logger.warn('Custom WhatsApp send did not succeed', {
      enquiryId: data.id,
      customerPhone: phone,
      to: result.to,
      error: result.error,
