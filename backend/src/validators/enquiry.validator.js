...
const digitsPhoneRegex = /^\d{10,15}$/;

const enquiryBodyBase = {
  enquiry_type: z.enum(ENQUIRY_TYPES),
  customer_name: z.string().min(1, 'Name is required'),
  phone: z.string().regex(phoneRegex, 'WhatsApp number must be 10-15 digits; + is optional'),
  emergency_contact_number: z
    .union([
      z.string().regex(digitsPhoneRegex, 'Another contact must be 10-15 digits'),
      z.literal(''),
      z.null(),
    ])
    .optional(),
  email: z
    ...
    .optional(),
  country_id: z.union([z.string().uuid(), z.literal(''), z.null()]).optional(),
  ...
  approx_distance_km: z.preprocess((val) => {
  ...
  corporate_id: z.union([z.string().uuid(), z.literal(''), z.null()]).optional(),
  service_required: z
...
const updateEnquirySchema = z.object({
  ...
  body: z
    ...
    .superRefine((data, ctx) => {
      ...
      if (data.enquiry_type === 'agent' && !data.agent_id) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
      ...
      if (data.enquiry_type === 'corporate' && !data.corporate_id) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
const sendCustomerWhatsAppSchema = z.object({
  params: z.object({ id: z.string().uuid() }),
  body: z.object({
...
const vehicleAssignmentSchema = z.object({
  ...
  body: z.object({
    ...
    vendor_driver_name: z.preprocess(
...
const driverShareCodeSchema = z.object({
  params: z.object({
