...
function buildInvoiceRows({
  ...
  (quotations || []).forEach((q) => {
    ...
    const amount = Math.round(quotationAmount(q, itineraries) + extra);
  ...
  (itineraries || []).forEach((item) => {
    ...
    const amount = Math.round((summarizePricing(item.pricing || {}).grandTotal || 0) + extra);
    const balance = Math.max(amount - totalPaid, 0);
    const paidStatus =
      amount > 0 && balance <= 0 ? 'paid' : totalPaid > 0 ? 'partial' : 'unpaid';
...
export default function EnquiryInvoicePanel({ enquiry, itineraries = [] }) {
  ...
  const totalPaid = useMemo(
    () =>
      payments.reduce(
        (sum, p) => sum + (Number(p.advance_amount) || 0) + (Number(p.additional_charges) || 0),
        0
      ),
    [payments]
  );
  ...
  const rows = useMemo(
    ...
    [quotations, itineraries, enquiry, enquiryId, totalPaid, additionalCharges]
  );
