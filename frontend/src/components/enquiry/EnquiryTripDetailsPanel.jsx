...
function isTripVisible(assignment) {
...
function TripCard({ trip, enquiry }) {
  ...
  const driverName = trip.driver?.full_name || trip.vendor_driver_name || '—';
...
export default function EnquiryTripDetailsPanel({ enquiry }) {
  ...
  const trips = useMemo(
    () => (assignments || []).filter((row) => isTripVisible(row)),
    [assignments]
  );
...
export function enquiryHasTripDetails(assignments = []) {
  return (assignments || []).some((row) => isTripVisible(row));
}
