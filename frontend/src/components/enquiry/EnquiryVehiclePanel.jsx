...
import OwnershipTag, { ownershipType } from '../common/OwnershipTag';
...
function buildDriverShareLink(enquiry, assignment) {
  if (assignment?.share_code) {
    return `${window.location.origin}/d/${assignment.share_code}`;
  }
...
export default function EnquiryVehiclePanel({ enquiry }) {
  ...
  const isVendorVehicle = ownershipType(selectedVehicle) === 'vendor';
  const visibleAssignments = useMemo(
    () => (assignments || []).filter((row) => isVisibleAssignment(row)),
    [assignments]
  );
  ...
  const handleVehicleChange = (vehicleId) => {
    ...
    const vendor = ownershipType(vehicle) === 'vendor';
    setForm((prev) => {
      const next = { ...prev, vehicle_id: vehicleId };
      if (vendor) {
        next.driver_id = '';
      } else {
        next.vendor_driver_name = '';
        if (!prev.driver_id) {
  ...
  const saveAssignment = async () => {
    ...
    const vendor = ownershipType(vehicles.find((v) => v.id === form.vehicle_id)) === 'vendor';
    const vendorDriverName = String(form.vendor_driver_name || '').trim();
    if (vendor && !vendorDriverName) return;
    await create.mutateAsync({
      id: enquiryId,
      vehicle_id: form.vehicle_id,
      driver_id: vendor ? null : form.driver_id || null,
      vendor_driver_name: vendor ? vendorDriverName : null,
  ...
  const formValid =
    ...
    (!isVendorVehicle || Boolean(String(form.vendor_driver_name || '').trim()));
        ...
        {isLoading ? (
          ...
          <Typography variant="body2" color="text.secondary" sx={{ py: 2, textAlign: 'center' }}>
            {canEdit
              ...
              : 'No vehicle assigned to this enquiry.'}
          </Typography>
        ) : (
          <TableContainer
            sx={{
                  ...
                  <TableRow key={row.id} hover>
                    ...
                    <TableCell>
                      <Typography variant="body2" fontWeight={600}>
                        {row.driver?.full_name || row.vendor_driver_name || '—'}
                      </Typography>
                      {row.driver && (
                        <>
                          <Box sx={{ mt: 0.4 }}>
                      ...
                      {!row.driver && row.vendor_driver_name ? (
                        <Box sx={{ mt: 0.4 }}>
                    ...
                    <TableCell align="right">
                      <Stack direction="row" spacing={0.25} justifyContent="flex-end">
                        {row.driver_id && (
                          <Tooltip title="Share short driver login link">
                            <IconButton
                              size="small"
                              color="primary"
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              {isVendorVehicle ? (
                <TextField
                  ...
                  required
                  label="Driver *"
                  placeholder="Enter vendor driver name"
                  value={form.vendor_driver_name}
                  onChange={(e) => setField('vendor_driver_name', e.target.value)}
                  helperText="Vendor vehicle — type the driver name"
                />
              ) : (
                <TextField
                  select
      ...
      >
        ...
        <DialogContent>
          <Stack spacing={1.75} sx={{ pt: 0.5 }}>
            ...
            <TextField
              label="Short driver portal link"
              value={shareLink}
              fullWidth
              InputProps={{ readOnly: true }}
              helperText="Short link the driver can open to sign in for this trip"
            />
