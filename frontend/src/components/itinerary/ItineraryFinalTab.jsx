...
export default function ItineraryFinalTab({
  ...
  const pricing = useMemo(() => {
    const base = coercePricing(display?.pricing);
    return {
      ...base,
      line_items: resolveLineItems(days, base.line_items || []),
    };
  }, [display?.pricing, days]);
  ...
  const dateLabel =
    display?.from_date && display?.to_date
      ? `${dayjs(display.from_date).format('DD MMM YYYY')} – ${dayjs(display.to_date).format('DD MMM YYYY')}`
      : '';
      ...
      >
        <Box sx={{ p: { xs: 2.5, md: 3.5 } }}>
          ...
          <Typography sx={{ opacity: 0.95, fontWeight: 600 }}>
            {dateLabel}
            {dateLabel ? ' · ' : ''}
            {display?.days || data?.days || 0} Days / {display?.nights || data?.nights || 0} Nights · {routeLabel}
          </Typography>
          <Stack direction="row" spacing={1} mt={1.5} flexWrap="wrap" useFlexGap>
            <Chip
              label={`Adult: ${display?.adults ?? 1}`}
              size="small"
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff', fontWeight: 700 }}
            />
            <Chip
              label={`Child: ${display?.children ?? 0}`}
              size="small"
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff', fontWeight: 700 }}
            />
            <Chip
