...
export const useEnquiryPipelineStats = (params) =>
  useQuery({
    queryKey: ['enquiries', 'pipeline-stats', params],
    queryFn: async () => {
      const { data } = await enquiryService.pipelineStats(params);
      const payload = data?.data ?? data ?? {};
      return {
        total: Number(payload.total ?? payload.totalQueries ?? 0),
...
export const useExistingCustomers = (enabled = true) =>
  useQuery({
    queryKey: ['enquiries', 'customers'],
    queryFn: async () => {
      const { data } = await enquiryService.listCustomers();
      ...
      onError: (e) =>
        enqueueSnackbar(
          e?.response?.data?.message ||
            (e?.code === 'ECONNABORTED' ? 'Status update timed out. Refresh and try again.' : null) ||
...
export const useEnquiryWhatsAppMessages = (id, enabled = true) =>
  useQuery({
    queryKey: ['enquiry-whatsapp-messages', id],
    queryFn: async () => {
      const { data } = await enquiryService.listWhatsAppMessages(id);
      return data?.data ?? data ?? [];
    },
    enabled: !!id && enabled,
  });
