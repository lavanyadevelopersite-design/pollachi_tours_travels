...
import { useEnquiries, useEnquiryMutation, useEnquiryPipelineStats } from '../../hooks/queries/useEnquiry';
import { useLeadStatuses } from '../../hooks/queries/useMasters';
import { useUsers } from '../../hooks/queries/useUsers';
import enquiryService from '../../services/enquiry.service';
import EnquiryStatusRibbon from '../../components/enquiry/EnquiryStatusRibbon';
...
import { ENQUIRY_TYPES, getPublicEnquiryUrl } from '../../utils/constants';
...
export default function EnquiryList() {
  ...
  } = useEnquiryPipelineStats(pipelineParams);
  ...
  const leadStatusOptions = excludeCancelledLeadStatuses(leadStatusData?.rows || []);
  const users = usersData?.rows || [];
  const ribbonStatuses = useMemo(() => {
    const list = pipelineStats?.statuses?.length
      ? pipelineStats.statuses
    ...
    return excludeCancelledLeadStatuses(list);
  }, [pipelineStats, leadStatusOptions]);
  const ribbonTotal =
    pipelineStats?.total ??
  ...
  const setLeadStatusFilter = useCallback(
    (value) => {
      setStatusFilter(value);
      setPage(1);
      const next = new URLSearchParams(searchParams);
      if (value) next.set('lead_status_id', value);
      else next.delete('lead_status_id');
  ...
  const applyStatusFilter = useCallback(
    (value) => {
      setLeadStatusFilter(value === statusFilter ? '' : value);
    },
  ...
  const openAssignDialog = useCallback((row) => {
    setAssignRow(row);
    setAssignUserId(row.assigned_to || row.assignee?.id || '');
  }, []);
  ...
  const saveAssign = async () => {
    if (!assignRow) return;
    const assignedTo = assignUserId || null;
  ...
  const copyCustomerFormLink = async () => {
    const link = getPublicEnquiryUrl();
    try {
      await navigator.clipboard.writeText(link);
      enqueueSnackbar('Customer enquiry form link copied', { variant: 'success' });
    } catch {
  ...
  const shareFeedbackWhatsApp = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['enquiry-whatsapp-messages', feedbackRow.id] });
    } catch (err) {
      enqueueSnackbar(
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        minWidth: '140px',
        cell: (r) => {
          const cancelled = isEnquiryActionsLocked(r) && /cancell?ed/i.test(String(r.status || r.leadStatus?.lead_status || ''));
          const label = cancelled ? 'Cancelled' : r.leadStatus?.lead_status || '—';
          const color = cancelled ? '#dc2626' : r.leadStatus?.button_color || '#90a4ae';
          return (
            <Button
              size="small"
              disableElevation
      ...
      },
      {
        ...
        cell: (row) => (
          <Box sx={{ display: 'flex', flexWrap: 'nowrap' }}>
            ...
            {canEdit && !row.is_converted && !isEnquiryActionsLocked(row) && (
            ...
            {canDelete && !isEnquiryRecordLocked(row) && (
    ...
    [canEdit, canDelete, canFeedback, navigate, convert, openStatusDialog, openAssignDialog, openFeedbackShare, canCancelEnquiry]
  );
  ...
  return (
    <Box>
      ...
      <EnquiryStatusRibbon
        total={ribbonTotal}
        statuses={ribbonStatuses}
        selectedId={statusFilter}
        onSelect={applyStatusFilter}
        loading={pipelineLoading && !pipelineStats}
      />
      <DataTable
        title="Enquiries"
        columns={columns}
      ...
      <ConfirmDialog
        ...
        message={`Cancel enquiry ${cancelRow?.enquiry_code || ''}? Follow-up, itinerary, and vehicle actions will be locked.`}
        confirmLabel="Cancel Enquiry"
        onCancel={() => setCancelRow(null)}
        onConfirm={async () => {
          if (!cancelRow?.id) return;
          await updateStatus.mutateAsync({ id: cancelRow.id, status: 'cancelled' });
          setCancelRow(null);
        }}
      ...
      <Dialog open={!!assignRow} onClose={closeAssignDialog} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ pt: 1 }}>
          ...
          <Typography variant="caption" color="text.secondary">
            Assigning a user moves the lead status to Assign Enquiry when it is still New Enquiry.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={saveAssign} disabled={update.isPending || !assignUserId}>
