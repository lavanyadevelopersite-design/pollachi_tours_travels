import { useCallback, useMemo, useState } from 'react';
import {
  Box,
  IconButton,
  Tooltip,
  FormControl,
  InputLabel,
  Select,
  MenuItem,
  Switch,
} from '@mui/material';
import EditIcon from '@mui/icons-material/Edit';
import DeleteIcon from '@mui/icons-material/Delete';
import PageHeader from '../../components/common/PageHeader';
import DataTable from '../../components/dataTable/DataTable';
import ConfirmDialog from '../../components/common/ConfirmDialog';
import StatusBadge from '../../components/common/StatusBadge';
import Loader from '../../components/common/Loader';
import { usePermission } from '../../hooks/usePermission';
import { useCurrencies, useCurrencyMutation } from '../../hooks/queries/useMasters';
import { activeStatus } from '../../schemas/master.schema';
import CurrencyForm from './CurrencyForm';

export default function CurrencyList() {
  const canCreate = usePermission('currencies.create');
  const canEdit = usePermission('currencies.edit');
  const canDelete = usePermission('currencies.delete');
  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(10);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('');
  const [sortBy, setSortBy] = useState('created_at');
  const [sortOrder, setSortOrder] = useState('desc');
  const [deleteId, setDeleteId] = useState(null);
  const [dialog, setDialog] = useState({ open: false, mode: 'create', row: null });

  const listParams = useMemo(() => {
    const params = { page, perPage, search, sortBy, sortOrder };
    if (statusFilter === 'active') params.is_active = true;
    if (statusFilter === 'inactive') params.is_active = false;
    return params;
  }, [page, perPage, search, sortBy, sortOrder, statusFilter]);

  const { data, isLoading } = useCurrencies(listParams);
  const { create, update, remove, updateStatus } = useCurrencyMutation();
  const rows = data?.rows || [];
  const total = data?.pagination?.total || rows.length;

  const handleSearch = useCallback((value) => {
    setSearch(value);
    setPage(1);
  }, []);

  const closeDialog = () => setDialog({ open: false, mode: 'create', row: null });

  const columns = useMemo(
    () => [
      {
        id: 'sno',
        name: 'S.No',
        width: '70px',
        cell: (_row, index) => (page - 1) * perPage + index + 1,
      },
      { id: 'name', name: 'Name', selector: (r) => r.name, sortable: true, sortField: 'name' },
      {
        id: 'exchange_rate',
        name: 'Rate',
        selector: (r) => r.exchange_rate,
        sortable: true,
        sortField: 'exchange_rate',
      },
      {
        id: 'status',
        name: 'Status',
        cell: (r) => (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <StatusBadge status={activeStatus(r)} />
            {canEdit && (
              <Switch
                size="small"
                checked={r.is_active !== false}
                onChange={() =>
                  updateStatus.mutate({ id: r.id, is_active: !(r.is_active !== false) })
                }
              />
            )}
          </Box>
        ),
      },
      {
        id: 'actions',
        name: 'Actions',
        width: '120px',
        omitExport: true,
        cell: (row) => (
          <Box>
            {canEdit && (
              <Tooltip title="Edit">
                <IconButton
                  size="small"
                  color="warning"
                  onClick={() => setDialog({ open: true, mode: 'edit', row })}
                >
                  <EditIcon fontSize="small" />
                </IconButton>
              </Tooltip>
            )}
            {canDelete && (
              <Tooltip title="Delete">
                <IconButton size="small" color="error" onClick={() => setDeleteId(row.id)}>
                  <DeleteIcon fontSize="small" />
                </IconButton>
              </Tooltip>
            )}
          </Box>
        ),
      },
    ],
    [canEdit, canDelete, page, perPage, updateStatus]
  );

  const filters = (
    <FormControl size="small" sx={{ minWidth: 160 }}>
      <InputLabel id="currency-status-filter">Status</InputLabel>
      <Select
        labelId="currency-status-filter"
        value={statusFilter}
        label="Status"
        onChange={(e) => {
          setStatusFilter(e.target.value);
          setPage(1);
        }}
      >
        <MenuItem value="">All</MenuItem>
        <MenuItem value="active">Active</MenuItem>
        <MenuItem value="inactive">Inactive</MenuItem>
      </Select>
    </FormControl>
  );

  if (isLoading && !data) return <Loader />;

  return (
    <Box>
      <PageHeader
        title="Currency"
        subtitle="Manage currencies used across the application"
        actionLabel={canCreate ? 'Add Currency' : undefined}
        actionPermission={canCreate}
        onAction={() => setDialog({ open: true, mode: 'create', row: null })}
      />
      <DataTable
        title="Currency"
        columns={columns}
        data={rows}
        totalRows={total}
        page={page}
        perPage={perPage}
        onPageChange={setPage}
        onPerPageChange={(value) => {
          setPerPage(value);
          setPage(1);
        }}
        searchValue={search}
        onSearch={handleSearch}
        onSort={(field, direction) => {
          setSortBy(field);
          setSortOrder(direction);
        }}
        tableKey="currencies"
        exportFilename="currencies"
        exportable={false}
        paginationServer
        loading={isLoading}
        filters={filters}
      />
      <CurrencyForm
        open={dialog.open}
        mode={dialog.mode}
        initialData={dialog.row}
        loading={create.isPending || update.isPending}
        onCancel={closeDialog}
        onSubmit={async (payload) => {
          if (dialog.mode === 'edit' && dialog.row?.id) {
            await update.mutateAsync({ id: dialog.row.id, ...payload });
          } else {
            await create.mutateAsync(payload);
          }
          closeDialog();
        }}
      />
      <ConfirmDialog
        open={!!deleteId}
        title="Delete Currency"
        message="Are you sure you want to delete this currency?"
        onCancel={() => setDeleteId(null)}
        onConfirm={async () => {
          await remove.mutateAsync(deleteId);
          setDeleteId(null);
        }}
        confirmLabel="Delete"
      />
    </Box>
  );
}
