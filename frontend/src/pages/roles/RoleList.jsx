...
export default function RoleList() {
  ...
  const { create, update, remove } = useRoleMutation();
  ...
  const handleDeleteFromForm = () => {
    ...
    setFormOpen(false);
    setDeleteRole(editingRole);
    setEditingRole(null);
  };

  const handleDeleteRole = async () => {
    if (!deleteRole?.id) return;
    await remove.mutateAsync(deleteRole.id);
    ...
    <Box>
                  ...
                  <Box key={role.id}>
                      ...
                      secondaryAction={
                        (canEdit || canDelete) ? (
                          <IconButton
                            ...
                            onClick={(e) => openRoleMenu(e, role)}
                            sx={{
                              color: selected ? 'primary.main' : 'text.secondary',
                              '&:hover': { bgcolor: 'rgba(33,150,243,0.12)', color: 'primary.main' },
                            }}
                          >
                            <MoreVertIcon />
                          </IconButton>
                    ...
                    >
                      ...
                      >
                        ...
                        <ListItemText
                          primary={
                            <Typography fontWeight={selected ? 700 : 600} fontSize={14} noWrap>
                              {role.name}
                            </Typography>
                          }
                          secondary={
      ...
      >
        {canEdit && (
          ...
          >
            <EditOutlinedIcon sx={{ mr: 1.5, fontSize: 20 }} />
            Edit
          </MenuItem>
        )}
        {canDelete && (
          <MenuItem
            ...
            onClick={() => {
              const role = menuRole;
              closeRoleMenu();
              if (role) setDeleteRole(role);
            }}
          >
            <DeleteOutlinedIcon sx={{ mr: 1.5, fontSize: 20 }} />
            Delete
          </MenuItem>
        )}
      </Menu>
      ...
      <ConfirmDialog
        open={!!deleteRole}
        title="Delete Role"
        message={`Are you sure you want to delete the role "${deleteRole?.name}"? This cannot be undone.`}
        confirmLabel="Delete"
