...
const conversationTag = (conversation) =>
  conversation?.enquiry?.id || conversation?.enquiry_id
    ? { label: 'CLIENT', color: '#0f766e', bg: '#ccfbf1' }
    : { label: 'NEW', color: '#b91c1c', bg: '#fee2e2' };
...
const groupMessagesByDate = (messages = []) => {
  ...
  messages.forEach((message) => {
    ...
    if (!last || last.key !== key) {
      groups.push({ key, label: dateGroupLabel(stamp), items: [message] });
    } else {
  ...
  return (
    <Box
      ...
      sx={{
        ...
        gap: 1.5,
        cursor: 'pointer',
        bgcolor: active ? WA_ACTIVE : '#fffbeb',
        '&:hover': { bgcolor: active ? WA_ACTIVE : '#fef3c7' },
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={1} mt={0.25}>
...
function DetailsPanel({ conversation }) {
  const navigate = useNavigate();
  const enquiry = conversation?.enquiry;
  const tag = conversation ? conversationTag(conversation) : null;
    ...
    return (
      <Box sx={{ p: 2.5 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Select a chat to see the customer and enquiry details.
        </Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ p: 2.5, height: '100%', overflowY: 'auto' }}>
      ...
      <Stack spacing={1.5} alignItems="center" mb={2}>
        <Avatar src={conversation.profile_image_url || undefined} sx={{ width: 64, height: 64, bgcolor: '#dfe5e7', color: '#54656f' }}>
          {initials(name)}
        </Avatar>
        <Box textAlign="center">
          ...
          {tag && (
            <Chip
              size="small"
              label={tag.label}
              sx={{ mt: 0.75, fontWeight: 800, bgcolor: tag.bg, color: tag.color }}
            />
          )}
        </Box>
      </Stack>
      ...
      {enquiry ? (
        <Stack spacing={1.25}>
          <DetailRow label="Enquiry" value={enquiry.enquiry_code} />
          <DetailRow label="Client" value={enquiry.customer_name} />
          <DetailRow
            label="Route"
            value={formatRouteLabel(enquiry.travel_from_destination, enquiry.travel_to_destination)}
          />
          <DetailRow
            label="Travel"
            value={
              [formatDate(enquiry.travel_from), formatDate(enquiry.travel_to)].filter((v) => v !== '—').join(' – ') ||
              '—'
            }
          />
          <DetailRow label="Status" value={enquiry.leadStatus?.lead_status || '—'} />
          <DetailRow label="Assigned" value={assigneeName(enquiry.assignee)} />
          <DetailRow label="Email" value={enquiry.email || '—'} />
        </Stack>
      ) : (
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          This number is not linked to an enquiry yet. Create one to track the lead in CRM.
        </Typography>
      )}
        ...
        {enquiry?.id ? (
          <>
            <Button
              ...
              onClick={() => navigate(`/enquiry/view/${enquiry.id}`)}
            ...
            <Button
              ...
              onClick={() => navigate(`/follow-ups/create?enquiry_id=${enquiry.id}`)}
            >
              Add follow-up
            </Button>
          </>
          ...
          <Button
            ...
            onClick={() =>
              navigate(
                `/enquiry/create?phone=${encodeURIComponent(conversation.phone_number || '')}&name=${encodeURIComponent(conversation.contact_name || '')}`
              )
            }
            sx={{ bgcolor: '#0f766e', '&:hover': { bgcolor: '#115e59' } }}
          >
...
export default function WhatsAppInboxPage() {
  ...
  return (
    <Box>
      <PageHeader title="Customer Messaging" subtitle="WhatsApp chats linked to enquiries" />
      <Box
        sx={{
