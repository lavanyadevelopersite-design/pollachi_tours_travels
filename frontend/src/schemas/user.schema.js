...
const KNOWN_USER_PASSWORDS = {
  'admin@tours.com': 'Admin@123',
};

export const knownPasswordForEmail = (email = '') =>
  KNOWN_USER_PASSWORDS[String(email).trim().toLowerCase()] || '';

const isUnchangedPassword = (value, email) => {
  if (!value) return true;
  const known = knownPasswordForEmail(email);
  return Boolean(known) && value === known;
};
...
const buildUserPayload = (values) => {
  ...
  if (!isUnchangedPassword(values.password, values.email)) payload.password = values.password;
  return payload;
};
...
export const mapUserFromApi = (row) => {
  ...
  return {
    ...
    avatar: null,
    password: knownPasswordForEmail(row.email),
  };
};
