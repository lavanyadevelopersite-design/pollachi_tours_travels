    ...
    <Box>
      ...
      {helperText && (
        <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
          {disabled ? 'Starting KM and screenshot are locked after save.' : helperText}
        </Typography>
      )}
    </Box>
  );
...
export default function DriverTripDetail() {
  ...
  const validateStartingKm = () => {
    if (startingKmLocked) return true;
    if (startingKm === '' || Number.isNaN(Number(startingKm))) {
      enqueueSnackbar('Please enter starting KM', { variant: 'warning' });
      return false;
    }
    if (!hasStartingPhoto) {
      enqueueSnackbar('Please upload starting KM screenshot', { variant: 'warning' });
      return false;
    }
    return true;
  };
  ...
  const validateClosingKm = () => {
    if (closingKm === '' || Number.isNaN(Number(closingKm))) {
      enqueueSnackbar('Please enter closing KM', { variant: 'warning' });
      return false;
    }
    return true;
  };
  ...
  const applyStatus = (statusDef) => {
    ...
    if (statusDef.needsStartingKm) {
      ...
      if (!startingKmLocked) payload.starting_km = Number(startingKm);
    }
    ...
    <Stack spacing={2}>
      ...
      >
        ...
        <Stack spacing={1.5}>
          <TextField
            ...
            fullWidth
            required={!startingKmLocked}
            disabled={startingKmLocked}
          ...
          <TextField
            ...
            fullWidth
            inputProps={{ min: 0, step: '0.1', inputMode: 'decimal' }}
            helperText="Enter odometer reading when trip ends (required only when closing the trip)"
          />
          <KmPhotoUpload
      ...
      >
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Tap the next step only. Completed steps cannot be changed back.
        </Typography>
