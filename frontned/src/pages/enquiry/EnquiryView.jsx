...
export default function EnquiryView() {
  ...
  const leadStatuses = leadStatusData?.rows || [];
          ...
          <Card elevation={0} sx={panelSx}>
            ...
            <CardContent sx={{ pt: 2, pb: 2 }}>
              ...
              <Stack spacing={1}>
                ...
                {(enquiry.city_name || enquiry.city?.name || enquiry.state_name || enquiry.state?.name || enquiry.country?.name) && (
          ...
          <Card elevation={0} sx={{ ...panelSx, mb: 0 }}>
            ...
            <CardContent sx={{ minHeight: 420 }}>
                    ...
                    <Box>
                      ...
                      <Grid container spacing={2}>
                        {assignedItineraries.map((item) => {
                          ...
                          const adults = item.adults ?? enquiry.adults ?? 0;
                          const children = item.children ?? enquiry.children ?? 0;
                                ...
                                <Box sx={{ p: 1.75, flex: 1, display: 'flex', flexDirection: 'column' }}>
                                  <Stack spacing={0.65} sx={{ mb: 1.5 }}>
                                    ...
                                    <Typography variant="body2" fontWeight={600} color="#334155">
                                      Date:{' '}
                                      {item.from_date
                                        ? formatDate(item.from_date, 'DD MMM YYYY')
                                        : '—'}{' '}
                                      Till:{' '}
                                      {item.to_date
                                        ? formatDate(item.to_date, 'DD MMM YYYY')
                                        : '—'}
                                    </Typography>
                                    <Typography variant="body2" fontWeight={600} color="#334155">
                                      Created:{' '}
                                  ...
                                  <Stack spacing={1} sx={{ mt: 'auto' }}>
                                    ...
                                    {canItineraryDelete && !isConfirmed && (
                                      <Button
                                        fullWidth
                                        variant="outlined"
                                        color="error"
              ...
              {activeTabKey === 'followup' && (
                <Box sx={{ position: 'relative', pt: canFollowUpCreate ? 0.5 : 0 }}>
                  {canFollowUpCreate && (
                    <Button
                      variant="contained"
                      disableElevation
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" mb={1.5}>
            Select an itinerary from the module to add into this enquiry section.
          </Typography>
