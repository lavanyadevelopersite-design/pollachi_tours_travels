...
export const currencySchema = z.object({
  name: z.string().trim().min(1, 'Currency name is required').max(150),
...
export const departmentSchema = z.object({
  ...
  departmentCode: z.string().trim().min(1, 'Department code is required').max(50),
...
export const mapCurrencyToApi = (values) => ({
  ...
  decimal_places: values.decimalPlaces ?? 2,
  exchange_rate: values.exchangeRate === '' || values.exchangeRate == null ? null : Number(values.exchangeRate),
...
export const mapDepartmentToApi = (values) => ({
  ...
  department_code: values.departmentCode.trim().toUpperCase(),
